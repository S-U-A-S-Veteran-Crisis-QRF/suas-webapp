...
- **2026-07-31** — Printable hackathon flyer shipped: `/hackathon-flyer` (unlisted, `noindex`) renders a one-page US-Letter recruitment/sponsor sheet for the **Veterans Innovation Hackathon, Aug 28–30 2026 at Hacker Dojo** — roles needed, the 3-day arc, an app-demo callout pointing at `suasqrf.org/app`, and a single registration QR (`luma.com/Innovation4Veterans`) in a gold CTA band; pre-rendered `public/docs/hackathon-flyer-2026.pdf` (1 page, verified via headless Chromium). Per Jacob, the printed sheet carries **no 988 crisis block** — it is event outreach, not a site page; the site's `CrisisBar` is untouched and still renders on the flyer page itself
