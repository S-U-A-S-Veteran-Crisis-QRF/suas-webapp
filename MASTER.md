...
- **2026-08-14** — Grant program put on autopilot per Jacob's standing directive (all preparation autonomous; Jacob signs finals only): standing authorization added to `CLAUDE.md`, grant-pipeline SOP + live opportunity pipeline filed in the vault, weekly Monday-morning grant-scan routine created (cloud, push-notifies on new finds), and a live-verified discovery sweep run across federal/VA, California, veteran-foundation, corporate, and in-kind lenses
...
- [ ] **Review the grant pipeline note** (vault `05_Reference/`) — Claude preps every application; you only sign and submit finals (standing directive 2026-08-14)
