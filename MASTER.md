...
- **2026-07-31** — Printable hackathon flyer shipped: `/hackathon-flyer` (unlisted, `noindex`) renders a one-page US-Letter recruitment/sponsor sheet for the **Veterans Innovation Hackathon, Aug 28–30 2026 at Hacker Dojo** — roles needed, the 3-day arc, an app-demo callout, and a four-QR scan row (Luma registration, `suasqrf.org/app`, `suasqrf.org`, `suasqrf.org/donate`); pre-rendered `public/docs/hackathon-flyer-2026.pdf` (1 page, verified via headless Chromium). Per Jacob, the printed sheet carries **no 988 crisis block** — it is event outreach, not a site page; the site's `CrisisBar` is untouched and still renders on the flyer page itself
...
- **SessionStart hook** — every session on every device auto-loads all of the above **and auto-catches-up on cross-device state** from the private sync channel (see [`docs/cross-device-sync.md`](docs/cross-device-sync.md)).
- **Cross-device session sync** — `/sync` skill + a `claude-sync` message-bus branch **on a private org repo** (session state never touches this public repo): sessions publish handoffs (`/sync handoff`) and notes that every other device receives automatically at session start. Entries are summaries — no PII/credentials/transcripts even privately.
