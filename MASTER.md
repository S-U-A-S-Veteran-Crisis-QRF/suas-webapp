...
- **2026-08-24** — Error/log sweep: the donate page's IRS determination-letter link was still 404ing on the live site — the PDF added 2026-07-20 was correct, but the plain `<a href="/docs/…">` gets no `basePath` rewrite from Next, so it resolved outside the `/suas-webapp` sub-path; now uses the `${base}` idiom. Also: `npm run lint` was a no-op (no ESLint installed — `next lint` fell into its interactive setup prompt and exited 0), replaced with a real flat-config ESLint run that immediately caught 6 errors (all fixed); `npm audit` back to 0 vulnerabilities (next 15.5.23, sharp forced to ^0.35.0 via `overrides`, avoiding the breaking Next 16 upgrade)
