...
- **2026-07-23** — Telegram bot built (`bot/`): a **stateless crisis-resource signpost** (not an AI counselor) running as a free Cloudflare Worker webhook — every reply leads with 988 press-1 / text 838255, then routes to real site content + a human. Commands: `/start` `/help` `/crisis` `/services` `/contact` `/donate` `/volunteer`; any other message → crisis line + "reach a person." Token kept out of the repo entirely (Worker secrets), webhook authenticated via `secret_token` header. Typecheck + local request/auth/routing tests pass; deploy runbook in `bot/README.md`. **Action for Jacob:** the BotFather token was shared in chat → revoke via @BotFather `/revoke`, then follow the runbook to deploy.
